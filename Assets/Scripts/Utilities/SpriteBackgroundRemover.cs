...
namespace GameIdle
{
    ...
    public static class SpriteBackgroundRemover
    {
        ...
        private static readonly Dictionary<Texture2D, Texture2D> _cacheDark = new();
        ...
        // Variante para ícones com fundo ESCURO e dessaturado (cinza/navy) — o
        // Process() padrão só limpa fundos claros. Faz flood-fill das bordas
        // limpando pixels escuros e sem cor, parando no ícone colorido/saturado.
        public static Texture2D ProcessDarkBg(Texture2D src)
        {
            if (src == null) return null;
            if (_cacheDark.TryGetValue(src, out var cached) && cached != null) return cached;

            Color32[] px;
            try { px = src.GetPixels32(); }
            catch { _cacheDark[src] = src; return src; }
            ...
            while (stack.Count > 0)
            {
                ...
                if (cur.a < 16 || IsDarkBackgroundish(cur))
                {
                    px[idx].a = 0;
                    TryNeighborDark(px, visited, stack, x - 1, y, w, h, cur);
                    TryNeighborDark(px, visited, stack, x + 1, y, w, h, cur);
                    TryNeighborDark(px, visited, stack, x, y - 1, w, h, cur);
                    TryNeighborDark(px, visited, stack, x, y + 1, w, h, cur);
                }
            }
        ...
        private static void TryNeighborDark(Color32[] px, bool[] visited, Stack<int> stack,
            int x, int y, int w, int h, Color32 from)
        {
            ...
            Color32 c = px[idx];
            if (c.a < 16 || IsDarkBackgroundish(c) || ColorDist(c, from) <= StepTolerance)
            {
                visited[idx] = true;
                stack.Push(idx);
            }
        ...
        private static bool IsDarkBackgroundish(Color32 c)
        {
            float r = c.r / 255f, g = c.g / 255f, b = c.b / 255f;
            float maxC = Mathf.Max(r, Mathf.Max(g, b));
            float minC = Mathf.Min(r, Mathf.Min(g, b));
            float sat = maxC > 0.001f ? (maxC - minC) / maxC : 0f;
            // Fundo = ESCURO e sem cor (cinza/navy). O gate de brilho impede de
            // comer partes coloridas/claras do ícone.
            return maxC < 0.42f && sat < 0.22f;
        }
